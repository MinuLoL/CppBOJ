...
int N,M;
int maps[1001][1001];
bool visited[1001][1001][2]={0,};	//visited[y][x][벽 부순 횟수]
...
int bfs(int y,int x) 
{
	queue<pair<pair<int,int>,pair<int,int> > > q;	//({y,x},{벽 부순 횟수,현재 이동한 칸 수})
	...
	while(!q.empty())
	{
		...
		for(int i=0;i<4;++i)
		{
			...
			if(isInMap(ny,nx))
			{
				if(maps[ny][nx]==1&&Breakcnt==0)//다음 이동하려는 칸이 벽이고 아직 부순적이없으면 
				{
					visited[ny][nx][Breakcnt+1]=true;
					q.push(make_pair(make_pair(ny,nx),make_pair(Breakcnt+1,cnt+1)));
				}
				else if(maps[ny][nx]==0&&visited[ny][nx][Breakcnt]==false)//다음 이동하려는 칸이 빈칸 일때 
				{
